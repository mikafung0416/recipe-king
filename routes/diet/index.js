...
router.post("/", async (req, res) => {
  ...
  console.log(diet);
  const url = `https://api.spoonacular.com/recipes/complexSearch?apiKey=${process.env.API_KEY3}&diet=${diet}&number=2`;
...
router.post("/:dietName", async (req, res, next) => {
  try {
    ...
    let url = `https://api.spoonacular.com/recipes/complexSearch?apiKey=${process.env.API_KEY3}&diet=${country}&number=50`;
...
router.post("/:dietName/number", async (req, res) => {
  ...
  const url = `https://api.spoonacular.com/recipes/complexSearch?apiKey=${process.env.API_KEY3}&diet=${dietName}&number=${numOfRecipes}`;
  const response = await fetch(url);
  const result = await response.json();
