...
router.post("/:cuisineName", async (req, res, next) => {
  try {
    ...
    let url = `https://api.spoonacular.com/recipes/complexSearch?apiKey=${process.env.API_KEY3}&cuisine=${country}&number=50`;
    console.log(url);
    ...
    //     if (instructionAnal.length !== null) {
...
//       "I just take the total recipes from api, so it may still render by db"
//     );
//     let url = `https://api.spoonacular.com/recipes/complexSearch?apiKey=${process.env.API_KEY3}&cuisine=${country}&number=2`;
//     let response = await fetch(url);
//     let result = await response.json();
//     let recipes = result.results;
//     let numOfRecipes = result.number;
...
//Range bar for numbre of recipes of cuisine, calling from db
router.post("/:cuisineName/number", async (req, res, next) => {
  try {
    ...
    if (recipeCuisineData.length < numOfRecipes) {
      ...
      let url = `https://api.spoonacular.com/recipes/complexSearch?apiKey=${process.env.API_KEY3}&cuisine=${country}&number=${numOfRecipes}`;
      let response = await fetch(url);
      let result = await response.json();
      let recipes = result.results;
      // console.log(recipes);
